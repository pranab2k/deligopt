...
const FoodDetails = () => {
  ...
  const fetchCategories = async (catid) => {
    ...
    try {
      const response = await fetch(`${BASE_URL}/homecategorylist.php?catid=${catid}`);
  ...
  const fetchRecommendedProduct = async (catid) => {
    try {
      const response = await fetch(`${BASE_URL}/homepageproductlist.php?catid=${catid}&userid=${userData?.userId || '0'}`);
      const json = await response.json();
      setLoading(true);
      if (json.success) {
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <Category
              ...
              onPress={() => {
                navigation.navigate('productlist', { categoriesid: item.catid });
              }}
            />
          )}
        />
      ...
      {atToCart > 0 && (
        <View style={styles.bookBottomContainer}>
          <TouchableOpacity
            ...
            onPress={() => navigation.navigate("cart/checkoutorders")}
          >
