...
const ActiveOrders = () => {
  ...
  const [useeId, setUserId] = useState('0'); // Default to '0' if userId is not available
  const userId = userData?.userId; // Default to '0' if userId is not available
...
const productData = {
      orderid: value, // You can replace this with dynamic user ID    
    };

    try {
      const response = await fetch(`${BASE_URL}/orderaccept.php`, {
        method: 'POST',
        headers: {
...
const router = useRouter();
  ...
  const fetchMyOderList = async (userid: string) => {
    ...
    try {
      const response = await fetch(`${BASE_URL}/myoderlist.php?userid=${userid}`);
        ...
        renderItem={({ item }) => (
            <TouchableOpacity style={[styles.cardContainer, {
              ...
              <View style={styles.buttonContainer}>
                {/* <TouchableOpacity
                 onPress={() => handleSubmit(item.order_id)}
                  //onPress={() => refRBSheet.current.open()}
                  style={styles.cancelBtn}>
                  <Text style={styles.cancelBtnText}>Accept Order</Text>
                </TouchableOpacity> */}
                <TouchableOpacity
